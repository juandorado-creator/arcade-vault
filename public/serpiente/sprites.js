...
window.SPRITE_ATLAS = {
  sources: {
    fruits: '/serpiente/fruits.png', // bg transparente
  },
...
window.FRUIT_NAMES = Object.keys(window.SPRITE_ATLAS.fruits);
let fruitsImg = null;
let fruitsLoaded = false;
const fruitsCallbacks = [];
function loadFruitSheet(cb) {
  if (fruitsLoaded) {
    cb();
    return;
  }
...
function drawFruit(ctx, name, x, y, w, h) {
  if (!fruitsLoaded) return;
  const sp = window.SPRITE_ATLAS.fruits[name];
  if (!sp) return;
  ctx.drawImage(fruitsImg, sp.x, sp.y, sp.w, sp.h, x, y, w, h);
}
